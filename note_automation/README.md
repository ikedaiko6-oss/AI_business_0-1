...
初回のみ依存インストール後、これを実行すると対話式でAPIキーを聞かれ、
動画取得〜記事生成〜アイキャッチ生成まで自動で完走します（**note.comへのログインは不要、投稿もしません**）。
生成された `output/article.md` と `output/eyecatch.png` を、あなた自身がnote.comにログインしてコピペ・アップロードしてください。

```bash
./run_pipeline.sh "https://youtube.com/shorts/xxxxx"
```

note.comへの下書き投稿まで自動化したい場合のみ `--publish` を付けてください（この場合だけNOTE_SESSION_COOKIEを聞かれます）:

```bash
./run_pipeline.sh "https://youtube.com/shorts/xxxxx" --publish
```
